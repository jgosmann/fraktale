 ...
 * Version: 2 (incompatible with version 1)                                *
...
struct gfxgui_button {
  public:
    ...
    gfxgui_button (BITMAP *normal, BITMAP *hover, BITMAP *selected,
                   ...
                   unsigned int _fnt_size, int label_x_offset = 0, int label_y_offset = 0,
...
gfxgui_button::gfxgui_button (BITMAP *normal, BITMAP *hover, BITMAP *selected,
                              ...
                              int label_x_offset, int label_y_offset,
                              void (*_onClick)(DIALOG *), void (*_onDeselect)(DIALOG *))
{
  bmp[0] = normal;
  bmp[1] = hover;
  bmp[2] = selected;
  bmp[3] = clicked;
  alfont_set_font_size(_lbl_fnt, _fnt_size);
  back = create_bitmap ((bmp[0]->w > alfont_text_length (_lbl_fnt, _label)
                                     + abs (label_x_offset))
                        ? bmp[0]->w : alfont_text_length (_lbl_fnt, _label)
                                       + abs (label_x_offset),
                        (bmp[0]->h > alfont_text_height (_lbl_fnt)
                                     + abs (label_y_offset))
                        ? bmp[0]->h : alfont_text_height (_lbl_fnt)
                                      + abs (label_y_offset));
	if (back == NULL)
        ...
        switch (obj->lbl_align)
          {
            ...
            case GFXGUI_TXT_ALIGN_CENTER:
               ...
               alfont_textout_centre_aa_ex (gui_get_screen (), obj->lbl_fnt, obj->label,
                                            d->x + (d->w / 2) + obj->lbl_x,
                                            d->y + obj->lbl_y, draw_col, makeacol (0, 0, 0, 255));
              break;
